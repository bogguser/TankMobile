 ...
 * 
 */
class QUESTSYSTEM_API IIInteractableObject
{
	GENERATED_BODY()
...
public:
	...
	void Interact(AActor* ActorInteractionWithObject);

	UFUNCTION()
	virtual void Interact_Implementation(AActor* ActorInteractionWithObject) {};

	UFUNCTION()
	virtual void NotifyInteractionFinished(AActor* InteractableObject, AActor* ActorInteractionWithObject)
	{
		if(OnInteractionFinished.IsBound())
		{
			OnInteractionFinished.Broadcast(InteractableObject, ActorInteractionWithObject);
		}
	}
